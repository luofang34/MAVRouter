...
async fn test_udp_echo() {
    ...
    tokio::spawn(async move {
        udp::run(1, "127.0.0.1:14550".to_string(), EndpointMode::Server, bus_tx, bus_rx, rt, dd, f, t).await.unwrap();
    });
    ...
    tokio::spawn(async move {
        tcp::run(2, "127.0.0.1:15760".to_string(), EndpointMode::Server, bus_tx2, bus_rx2, rt2, dd2, f2, t2).await.unwrap();
    });
...
async fn test_tcp_bidirectional() {
    ...
    tokio::spawn(async move {
        tcp::run(1, "127.0.0.1:15761".to_string(), EndpointMode::Server, bus_tx, bus_rx, rt, dd, f, t).await.unwrap();
    });
    ...
    tokio::spawn(async move {
        tcp::run(2, "127.0.0.1:15762".to_string(), EndpointMode::Server, bus_tx2, bus_rx2, rt2, dd2, f2, t2).await.unwrap();
    });
    ...
    for _ in 0..5 {
        ...
        // Short timeout read
        if let Ok(Ok(n)) = tokio::time::timeout(Duration::from_millis(500), client3.read(&mut rx_buf)).await {
            if n > 0 {
