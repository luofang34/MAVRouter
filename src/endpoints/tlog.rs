...
pub async fn run(
    ...
    if !dir.exists() {
        fs::create_dir_all(dir).await.context("Failed to create logs directory")?;
    }

    let now = SystemTime::now();
    let since_the_epoch_us = now.duration_since(UNIX_EPOCH)
    ...
    let file = File::create(&path).await.context("Failed to create log file")?;
